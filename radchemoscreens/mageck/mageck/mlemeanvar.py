...
class MeanVarModel:
    ...
    list_lowess = None
    # used to identify the lowess regression values of log(variance+0.01)
    lowess_search = {}
    ...
    def save_k_residule_to_file(self, filename):
        ...
        for i in range(len(self.list_k)):
            print('\t'.join([str(self.list_k[i]), str(
                self.list_res[i])]), file=fileoh)
        fileoh.close()
    ...
    def get_lowess_var(self, klist, returnalpha=False):
        ...
        varvalue = np.array(
            [(lambda x: x if x >= 0.01 else 0.01)(t) for t in varvalue])
        #
        if returnalpha:

            alphavalue = [(varvalue[i]-klist[i])/(klist[i]**2)
                          for i in range(len(klist))]
            alphavalue = np.array(
                [(lambda x: x if x >= 0.01 else 0.01)(t) for t in alphavalue])
            return alphavalue
        else:
            return varvalue
    ...
    def model_mean_var_by_lm(self):
        ...
        k_log2 = np.array([k_log[i]
                          for i in range(len(var_log)) if var_log[i] > (-1)])
        var_log2 = np.array([var_log[i]
                            for i in range(len(var_log)) if var_log[i] > (-1)])
        if len(k_log2) > 20:
            (slope, intercept, r_value, p_value,
             std_err) = linregress(k_log2, var_log2)
        else:
            (slope, intercept, r_value, p_value,
             std_err) = linregress(k_log, var_log)
        ...
        for (gid, gsk) in allgenedict.items():
            ...
            if gsk.MAP_sgrna_dispersion_estimate != None:
                ...
                for i in range(len(gsk.sgrnaid)):
                    normalized_k_mean = np.mean(np.multiply(
                        inverse_size_f, [sg_k[i+k*n] for k in range(nallsample)]))
                    list_k.append(normalized_k_mean)

        combined_list = [[list_k[i], list_dispersion[i]] for i in range(len(list_k)) if math.isnan(
            list_k[i]) == False and list_dispersion[i] < 10 and math.isnan(list_dispersion[i]) == False]
