...
def sgRNAscore_piecewisenorm(score_list,gene_list,CN_arr,CN_genedict,selectGenes=False):
    ...
    def linreg_bp(bp):
    # regression model for genes w/ CN <= breakpoint
        #print('CN_vals '+str(len(CN_vals)))
        #print('score_vals '+str(len(score_vals)))
        #print('bp '+str(bp))
        
        #print('updated CN_vals '+str(len(CN_vals[CN_vals<=bp])))
        slope,intercept,r_value,p_value,std_err = \
            stats.linregress(CN_vals[CN_vals<=bp],score_vals[CN_vals<=bp])
        return (slope,intercept)
    ...
    try:
        ...
        logging.error('Error while performing regression using CNV. This usually happens when your provided CNV profile is largely normal, or is not in the format of log2 ratio (0 - no CNV change; -1 - heterozygous loss; -2 - homozygous loss; >0: amplification).')
        sys.exit(-1)
    # create piecewise linear regression model
    (slope,intercept) = linreg_bp(opt_bp)
